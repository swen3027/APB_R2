...
{
  tinysh_cmd_t *cm;
	MESSAGE("cmd->parent value: %x", cmd->parent);
  if(cmd->parent)
    {
	MESSAGE("if(cmd->parent) is true\n\r");
	cm=cmd->parent->child;
	...
	if(!cm)
        ...
        {
		MESSAGE("Traversing LL in cmd->parent\n\r");
          while(cm->next) cm=cm->next;
          cm->next=cmd;
		MESSAGE("LL traversed\n\r");
        }
    }
  else if(!root_cmd)
    {
    ...
    {
   	MESSAGE("Traversing LL\n\r");
	   cm=root_cmd;
      while(cm->next) cm=cm->next;
      cm->next=cmd;      
	MESSAGE("LL has been traversed\n\r");
    }
}
...
void vTinyshTask(void * pvParameters)
{
	...
	tinysh_add_command(&ls_cmd);
	tinysh_add_command(&mkdir_cmd);
	tinysh_add_command(&wr_cmd);
	{
