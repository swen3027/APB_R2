...
//0101 0000 0001 0101 0000 0001 0000 0101
...
unsigned long value;
	...
	char str[20];
	#define LAN8720A 0x0007C0F0
	if( ( (ulID1 << 16UL ) | ( ulID2 & 0xFFF0UL ) ) == LAN8720A )
	{
		UART0_PrintString("Device checked successfully\n\r");
	...
	{
		UART0_PrintString("Failed on checking device id\n\r");
	...
	else
	{
		...
		UART0_PrintString("Failed to setup link status\n\r");
	}
	...
	if( lReturn == pdPASS )
	{
		...
		UART0_PrintString("Webserver started successfully\n\r");
	}
...
unsigned short us;
long x, lDummy;
//	LPC_PINCON->PINSEL2 = 0x0000000; //Set to GPIO to toggle 
	...
	LPC_EMAC->MCFG = (1 << 3)| (1 << 5) | MCFG_RES_MII;	//Set clock to CPUCLK/44 (AHB CLK is 100MHz)
	vTaskDelay(emacSHORT_DELAY * 2);
	LPC_EMAC->MCFG = (1 << 3)|(1 << 5);
...
//	EMAC_
	...
	//	if(us == 0x0000)
	//		UART0_PrintString("us == 0x0000\n\r");
	//	sprintf(test_str, "us = %x\n\r", us);
	//	UART0_PrintString(test_str);
		if( !( us & MCFG_RES_MII ) )
		{
			/* Reset complete */
			break;
	...
	if(x == 100 || x == 99)
	{
		UART0_PrintString("Hardware reset didn't end\n\r");
	}
	else
	{
		UART0_PrintString("Hardware reset ended\n\r");
	}
}
/*-----------------------------------------------------------*/
...
char str[20];
	...
	{
		vTaskDelay( emacSHORT_DELAY * 5 );
		UART0_PrintString("Wrote to command reg requesting auto negotiation\n\r");
//		for( x = 0; x < 10; x++ )
///		{
			us = prvReadPHY( PHY_REG_BMSR, &lDummy );
			
			while(!( us & PHY_AUTO_NEG_COMPLETE ))
			{
				us = prvReadPHY(PHY_REG_BMSR, &lDummy);
			}
				sprintf(str, "BMSR: 0x%x\n\r", us);
				UART0_PrintString(str);
...
char str[20];
	...
	for( x = 0; x < 10; x++ )
	{
		...
		if( usLinkStatus & 0x04 ) //Check link status
		{
//			sprintf(str, "BMSR: 0x%x\n\r", usLinkStatus);
//			UART0_PrintString(str);
			/* Link NOT is established. */
			UART0_PrintString("Link Established\n\r");
			lReturn = pdPASS;
			break;
		}
...
long x;
	...
	if( x < lMaxTime )
	{
//		UART0_PrintString("prvWritePHY Success\n\r");
		return pdPASS;
	}
	else
	{
		UART0_PrintString("prvWritePHY Failed\n\r");
		return pdFAIL;
	}
}
/*-----------------------------------------------------------*/
...
const long lMaxTime = 10;
	...
	{
		*plStatus = pdFAIL;
		UART0_PrintString("prvReadPHY failed\n\r");
	}
//	UART0_PrintString("prvReadPHY exiting\n\r");
